...
(function () {
  ...
  // Interactive engines (flow animation, group chat) render a few strings
  // dynamically in JS rather than static HTML, and cache JSON data read from
  // data-* attributes — both need to react when the language toggle fires.
  // Each engine instance pushes a refresh callback here; the language
  // toggle (below) calls all of them after swapping attributes.
  ...
  /* Delegated so it keeps working on .term spans that get swapped in
     fresh by the language toggle (innerHTML replacement destroys the
     old nodes, so per-element listeners bound at load time would not
     survive a translation swap). Tooltip text is read live from
     data-definition on each hover instead of being cached. */
  ...
  document.addEventListener('mouseout', e => {
    const term = e.target.closest('.term');
    if (!term || !term._tooltipEl) return;
  ...
  function initChat(containerEl) {
    ...
    function updateProgress() {
      ...
      progressEl.textContent = currentLang() === 'zh'
        ? index + ' / ' + messages.length + ' 条消息'
    ...
    updateProgress();
    langRefreshers.push(updateProgress);
  }
  ...
  function initFlow(containerEl) {
    if (!containerEl) return;
    // Read fresh (not cached) so a data-steps-zh translation swap is
    // picked up by whichever step is currently on screen.
    function stepsData() { return JSON.parse(containerEl.dataset.steps || '[]'); }
    ...
    function updateProgress() {
      if (!progressEl) return;
      progressEl.textContent = currentLang() === 'zh'
        ? '第 ' + step + ' / ' + stepsData().length + ' 步'
        : 'Step ' + step + ' / ' + stepsData().length;
    }

    function startLabel() {
      return currentLang() === 'zh' ? '点击"下一步"开始' : 'Click "Next Step" to begin';
    }

    function refreshLabel() {
      const data = stepsData();
      if (labelEl) labelEl.textContent = step > 0 && data[step - 1] ? (data[step - 1].label || '') : startLabel();
      updateProgress();
    }
    ...
    updateProgress();
    langRefreshers.push(refreshLabel);
  }
  ...
  /* ── LANGUAGE TOGGLE (EN / 中文) ──────────────────────────────
     Two translation mechanisms, both driven by "-zh" markers so no
     English content ever needs to be duplicated by hand:

     1. `data-zh="<html>"` on an element swaps its innerHTML. The
        original (English) markup is snapshotted into `data-en` the
        first time applyLang runs.
     2. Any other attribute ending in "-zh" (e.g. `data-tooltip-zh`,
        `data-definition-zh`, `data-explanation-right-zh`) swaps the
        same-named attribute with the "-zh" suffix removed (e.g.
        `data-tooltip`, `data-definition`, `data-explanation-right`).
        This covers tooltip text, glossary definitions, and quiz/bug
        explanations, all of which are read live from their attribute
        at interaction time rather than cached at load.

     The chosen language is remembered in localStorage.             */
  const LANG_KEY = 'course-lang';
  const langEls  = $$('[data-zh]');
  const attrPairs = [];

  $$('*').forEach(el => {
    Array.from(el.attributes).forEach(attr => {
      if (attr.name === 'data-zh' || !attr.name.endsWith('-zh')) return;
      const base = attr.name.slice(0, -3);
      attrPairs.push({ el, base });
    });
  });
  ...
  attrPairs.forEach(({ el, base }) => {
    if (!el.hasAttribute(base + '-en')) el.setAttribute(base + '-en', el.getAttribute(base) || '');
  });

  function applyLang(lang) {
    document.documentElement.setAttribute('data-lang', lang);
    langEls.forEach(el => {
      const html = lang === 'zh' ? el.getAttribute('data-zh') : el.getAttribute('data-en');
      if (html != null) el.innerHTML = html;
    });
    attrPairs.forEach(({ el, base }) => {
      const val = lang === 'zh' ? el.getAttribute(base + '-zh') : el.getAttribute(base + '-en');
      if (val != null) {
        el.setAttribute(base, val);
        if (el._tooltipEl) el._tooltipEl.textContent = val;
      }
    });
